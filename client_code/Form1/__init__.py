...
class Form1(Form1Template):
  ...
  def _build_ui(self):
    ...
    try:
      logo = Image(source="_/theme/512x512OsloconWorkout.png")
      logo.height = 26
      app_bar.add_component(logo)
    except Exception:
      pass

    app_bar.add_component(
      Label(
        text="Oslocon Workouts",
        role="app-title",
        spacing_above="none",
        spacing_below="none",
      )
  ...
  def _load_bootstrap(self):
    ...
    self.content_host.add_component(
      CurrentWorkoutForm(bootstrap_payload=self.bootstrap),
      full_width_row=True,
    )