...
class CurrentWorkoutForm(CurrentWorkoutFormTemplate):
  ...
  def _build_ui(self):
    ...
    self.root.add_component(self.footer_wrap, full_width_row=True)
  ...
  def exercise_change(self, exercise_data=None, **event_args):
    ...
    form.set_event_handler("x-close-modal", self.close_modal)
    form.set_event_handler("x-choose-exercise", self.change_exercise_selected)
    self.open_modal(form)

  def change_exercise_selected(self, exercise_id=None, **event_args):
    slot_number = (self.modal_form.tag or {}).get("slot_number") if self.modal_form else None
