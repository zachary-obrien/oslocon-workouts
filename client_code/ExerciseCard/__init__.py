...
class ExerciseCard(ExerciseCardTemplate):
    ...
    def _build_ui(self):
        self.root = ColumnPanel(background="#141c26", foreground="#f3f6fb", border="1px solid #283548")
        self.add_component(self.root)

        self.header = GridPanel(background="#141c26")
        self.root.add_component(self.header, full_width_row=True)

        self.title_label = Label(text="", bold=True, font_size=20, foreground="#f3f6fb")
        self.pill_label = Label(text="", foreground="#97a5b7")
        self.menu_btn = Button(text="⋯", width=40)
        self.edit_btn = Button(text="Edit", visible=False)
        ...
        self.summary_label = Label(text="", foreground="#97a5b7")
        self.root.add_component(self.summary_label)

        self.info_panel = ColumnPanel(visible=False, background="#1b2634", border="1px solid #283548")
        self.root.add_component(self.info_panel, full_width_row=True)
    ...
    def render(self):
        ...
        self.pill_label.text = ex.get("muscle_group") or "Unassigned"
        ...
        if ex.get("status") == "skipped":
            self.root.background = "#2c1b1b"
        elif ex.get("status") == "completed":
            self.root.background = "#163127"
        else:
            self.root.background = "#141c26"

        if ex.get("is_unassigned"):
            self.summary_label.visible = True
            self.summary_label.text = "This slot is empty. Choose an exercise from the database."
            return
        ...
        prev = ex.get("previous_session")
        strong = ex.get("strongest_day")
        if prev or strong:
        ...
        if prev:
            self.info_panel.add_component(Label(text=f"Previous: Day {prev['day_code']} • {prev['status']}", foreground="#97a5b7"))
        if strong:
            self.info_panel.add_component(Label(text=f"Strongest: E1RM {strong.get('best_e1rm') or '—'}", foreground="#97a5b7"))
    ...
    def _collapsed_summary(self):
        ...
        avg_reps = round(sum(int(s.get('reps') or 0) for s in sets) / len(sets))
