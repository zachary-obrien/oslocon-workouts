...
ensure_preset_routine,
add_empty_slot,
add_workout_day as add_day_impl,
remove_workout_day as remove_day_impl,
move_slot as move_slot_impl,
remove_slot as remove_slot_impl,
)
...
def _empty_payload(user=None, selected_day_code=None):
  user = user or {}
  return {
    "resolvedUser": {
      "display_name": (user["display_name"] if user and user.get("display_name") else (user["email"].split("@")[0].title() if user and user.get("email") else "User")),
      "email": user["email"] if user and user.get("email") else "",
    },
    "current_day": selected_day_code or "",
    "next_scheduled_day": selected_day_code or "",
    "day_options": [],
    "can_remove_current_day": False,
    "exercises": [],
    "progression_settings": {
      "progress_every_n_qualifying_workouts": int(user["progress_every_n_qualifying_workouts"] or 3) if user else 3
    },
  }
...
def build_workout_payload(user, selected_day_code=None):
  ...
  days = get_active_days(user)
  if not days:
    return _empty_payload(user, selected_day_code)
  ...
  return {
    "resolvedUser": _safe_user_payload(user),
...
def assign_slot_exercise(day_code, slot_number, exercise_id):
  ...
  slot.update(
    exercise=exercise,
    uses_bodyweight=bool(exercise["uses_bodyweight_default"]),
    updated_at=now(),
  )
  return build_workout_payload(user, day_code)
...
def _build_completion_summary(user, completion_bucket, tile_states, completed_at):
  ...
  return {
    ...
    }.get(completion_bucket, "Great work"),
    "message": message,
...
def submit_workout(payload):
  ...
  day_code = payload.get("day_code")
  if not day_code:
    raise Exception("Missing workout day.")
  ...
  for ex in exercises_payload:
    ...
    if not exercise_id or slot_number is None:
      continue
